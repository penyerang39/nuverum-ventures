...
export default function ContactModal({ isOpen, onClose, prefilledEmail = '', prefilledSubject = '', calendlyReady = false }: ContactModalProps) {
  ...
  return (
    <>
      {/* Modal Container - use display:none until loaded once, then use opacity transitions */}
      <div 
        className={`fixed inset-0 z-50 ${
          isOpen ? 'pointer-events-auto overflow-y-auto' : 'pointer-events-none overflow-hidden'
        }`}
        style={{
          display: !hasLoadedOnce && !isOpen ? 'none' : 'block',
          transition: hasLoadedOnce ? 'opacity 300ms ease-out' : 'none'
        }}
        aria-hidden={!isOpen}
      ...
      >
        ...
        <div className="flex min-h-full items-center justify-center p-4 text-center relative z-10">
          <div 
            className={`w-full max-w-6xl transform overflow-hidden rounded-2xl bg-white p-8 text-left align-middle border border-muted transition-all duration-300 ${
              isOpen 
                ...
                <div className="bg-background rounded-lg overflow-hidden relative">
                  <iframe
                    ...
                    onLoad={() => {
                      ...
                      // Give Calendly's JS a moment to initialize after iframe load
                      setTimeout(() => setHasLoadedOnce(true), 1000)
